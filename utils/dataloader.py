...
class YoloDataset(Dataset):
    ...
    # 它将一张选定的图片和随机的3张图片进行随机裁剪, 再拼接到一张图上作为训练数据
    ...
    # 为了增加每个目标的正样本数量, 即每个点都增加了相邻网格对它的预测
    # 可以把每个网格看成4个部分, 若点落在网格右上角部分则由该网格上、右临近的两个网格共同预测...
    ...
    def get_target(self, targets):
        ...
        for l in range(num_layers):
            ...
            # anchors : 9, 2 (每个特征图对应三种尺寸的先验框, 三个维度的特征图就是9)
            ...
            for t, ratio in enumerate(max_ratios):
                ...
                for k, mask in enumerate(self.anchors_mask[l]):
                    ...
                    for offset in offsets:
                        ...
                        # 把真实框的坐标放入y_true里面, t_x、t_y代表中心调整参数的真实值
